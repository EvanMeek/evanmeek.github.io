...
** 相对定位
   ...
   #+begin_src html
    <html>
      ...
      <body>
        <!-- relative 设置目标 -->
        <div style="width: 640px; height: 200px; background: #f2f2f0; position: relative;"></div>
        <!-- absolute 设置目标 | top / bottom / left / right -->
...
** 绝对定位
   ...
   #+end_src
** z-index
   设置被覆盖标签的层级关系，有点类似ps的图层的层级关系， ~z-index~ 值越大越上层，最低值为1。
   #+begin_src html
     <html>
       <style type="text/css">
         ,*{
...
** 多列局部
   ...
   #+end_src
* Flex
  Flex相比浮动和定位进行布局的方式更加灵活，也更适用于移动端应用。
** 空间分割
   #+begin_src html
     <html>
       <style type="text/css">
